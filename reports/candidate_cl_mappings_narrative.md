> Archived pre-migration rationale. Current decisions are in mappings/curated_mappings.tsv; the generated candidate report is authoritative for the current proposal set.
...
| SOULCAP | Proposed CL term | Proposed CL ID | Match type | Source |
|---|---|---|---|---|
| Mono | monocyte | `CL:0000576` | Exact | OAK-verified (exact label); OLS4 was timing out at lookup time; **sheet-confirmed 2026-08** |
| CMo | classical monocyte | `CL:0000860` | Exact | OAK-verified (exact label); **sheet-confirmed 2026-08** |
| NCMo | non-classical monocyte | `CL:0000875` | Exact | OAK-verified (exact label); **sheet-confirmed 2026-08** |
| IntMo | intermediate monocyte | `CL:0002393` | Exact | OAK-verified (exact label); **sheet-confirmed 2026-08** |
...
**Sheet confirmation (2026-09-08):** a fresh `soulcap-sync` found the master
Google Sheet's `OLS CL identifier` column — empty for every row as of July
21 — now independently populated for exactly these four rows, with these
exact four CL IDs. This is an outside curator (Kelly/David) confirming this
project's own proposal directly in the source of truth, not this project
editing the sheet. No other rows' `OLS CL identifier` changed in this diff.

This should be recorded as a distinct, high-confidence evidence source (a
curator sign-off in the actual target column, stronger than this project's
own lexical/marker inference) once the mapping table's evidence model is
upgraded — see the codex-owned implementation plan for this repo, which
already covers per-mapping evidence tracking. Not yet reflected as a
confidence value anywhere; this note is the durable record in the meantime.
