...
# ROBOT template
# --------------------------------------------------------------------------- #
def test_build_robot_template_rows_exact_uses_equivalent_to():
    rows = se.build_mapping_rows(
        [
            {
                "abbreviation": "X",
...
def test_write_robot_template_has_header_and_robot_row(tmp_path):
    ...
    out_path = tmp_path / "template.tsv"
    se.write_robot_template(out_path, rows)
...
def test_main_writes_robot_template_when_requested(tmp_path):
    ...
    template_path = tmp_path / "template.tsv"
    rc = se.main(
        [
            "--tsv",
            str(tsv),
